...
function amqpInit(config, cb) {
  ...
  amqp.connect(amqpUrl, function(err, connection) {
    ...
    cb(null, wrapConnection(connection, config.amqpExchange));
  });
}

function wrapConnection(connection, exchange) {
  return {

    createChannel: function(cb) {
      connection.createChannel(function(err, channel) {
        if (err) { return cb(err); }
        channel.assertExchange(exchange, 'topic', {},  function(err) {
          if (err) { return cb(err); }
          var wrapped = wrapChannel(channel, exchange);
...
function wrapChannel(channel, exchange) {
  return {
